...
/ / Release a binary tree memory non-recursive algorithm
...
private:
    ...
    typedef std::priority_queue<neighbor,
         ...
         // Search for the heap of the K-nearest neighbor (large top heap), the top of the heap is always the farthest point of the sample point in the K-nearest neighbor
    neighbor_heap k_neighbor_heap_;
         // p when looking for distance, dist(x, y) = pow((x^p + y^p), 1/p)
    float p;
         // Whether to release the memory of the tree when destructing
    bool free_tree_;
         // tree root node
    tree_node *root;
         // Training set 
    ...
    size_t n_samples;
         // the dimensions of each sample
    size_t n_features;
         // The label of the training set
    const float *labels;
         // The cache pool used to find the median
    std::tuple<size_t, float> *get_mid_buf_;
         // Search for the cache pool for K nearest neighbors, if you have searched for point i, make visited_buf[i] = True
    bool *visited_buf_;
 
#ifdef USE_INTEL_MKL
         // Cache when using the Intel MKL library
    float *mkl_buf_;
#endif
    ...
    void HeapStackPush(std::stack<tree_node *> &paths, tree_node *node, const float *coor, size_t k);
         ...
         // Get the value of the dim of the sample point in the training set
    float GetDimVal(size_t sample, size_t dim) {
        return datas[sample * n_features + dim];
    }
 
         // Find the distance from the coor to the i-th point of the training set
 ...
 // Find the K nearest neighbor of a tree. The id of Ki and the distance between Ki and coor are stored in args and dists respectively.
DLLExport
...
tree_model *build_kdtree(const float *datas, const float *labels,
 ...
 // Find the average for regression problems
float mean(const float *arr, size_t len) {
 ...
 // vote for classification problem
...
#ifdef USE_INTEL_MKL
         // To interact with C code, so apply for memory in C
